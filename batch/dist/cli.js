...
var ClickUpClient = class {
  constructor(config, fetchImpl = fetch, sleepImpl = defaultSleep) {
    this.config = config;
...
function applyMarker(body, markerBlock) {
  const stripped = body.replace(MARKER_RE, "");
  return `${stripped}${markerBlock}`;
}
var BATCH_WRITE_PACING_MS = 700;
async function runJob(job, opts, providedClient, sleep = (ms) => new Promise((resolve) => setTimeout(resolve, ms))) {
  const client = providedClient ?? new ClickUpClient(await loadConfig());
  const tasks = await client.paginate(
    `/list/${job.listId}/task`,
    "tasks",
    {}
  );
  const planned = job.computeChanges(tasks);
  const lines = [
    `# clickup-batch report - ${job.name}`,
    "",
  ...
  if (opts.write) {
    for (let i = 0; i < planned.length; i++) {
      await client.put(`/task/${planned[i].task_id}`, {
        description: planned[i].after
      });
      if (i < planned.length - 1) await sleep(BATCH_WRITE_PACING_MS);
    }
  }
  return { planned, report: lines.join("\n") + "\n" };
}

// ../batch/src/cli.ts
function parseArgs(argv) {
  const a = { write: false, out: "clickup-batch-report.md" };
  for (let i = 0; i < argv.length; i++) {
    if (argv[i] === "--write") a.write = true;
    else if (argv[i] === "--list" && argv[i + 1] !== void 0)
      a.listId = argv[++i];
    else if (argv[i] === "--text" && argv[i + 1] !== void 0)
      a.text = argv[++i];
    else if (argv[i] === "--out" && argv[i + 1] !== void 0)
      a.out = argv[++i];
  }
  return a;
}
async function main() {
  ...
  if (!args.listId || !args.text) {
    console.error(
      "Usage: clickup-batch --list <listId> --text <context> [--write] [--out file]\nDefault is DRY RUN. Re-runs replace the prior context block in place."
    );
    process.exit(2);
  }
...
---
## clickup-batch context (synced ${date})
${args.text}
`;
  const job = {
    name: "append-context",
    listId: args.listId,
    computeChanges(tasks) {
      return tasks.map((t) => {
        const before = t.description ?? "";
        return { task_id: t.id, before, after: applyMarker(before, block) };
      }).filter((c) => c.before !== c.after);
    }
  };
  const { planned, report } = await runJob(job, { write: args.write });
  writeFileSync(args.out, report, "utf8");
  console.error(
    `${args.write ? "Applied" : "Dry-run"} ${planned.length} changes. Report: ${args.out}`
  );
}
main().catch((e) => {
  console.error("clickup-batch FAILED:", e.message);
  process.exit(1);
});
